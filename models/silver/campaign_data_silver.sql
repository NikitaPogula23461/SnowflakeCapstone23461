...
cleaned_campaign AS (
    ...
    CASE
        WHEN TRY_TO_DATE(start_date) IS NOT NULL AND TRY_TO_DATE(end_date) IS NOT NULL THEN DATEDIFF(day, TRY_TO_DATE(start_date), TRY_TO_DATE(end_date))
        ELSE NULL
    ...
    CASE
        ...
        THEN(TRY_TO_NUMBER(REGEXP_REPLACE(total_revenue,'[^0-9.]','')) -TRY_TO_NUMBER(REGEXP_REPLACE(total_cost,'[^0-9.]',''))) / NULLIF(TRY_TO_NUMBER(REGEXP_REPLACE(total_cost,'[^0-9.]','')),0)
        ELSE NULL
    END AS expected_roi,
    ...
    CASE
        ...
        WHEN ABS(TRY_TO_NUMBER(roi_calculation) -((TRY_TO_NUMBER(REGEXP_REPLACE(total_revenue,'[^0-9.]','')) -TRY_TO_NUMBER(REGEXP_REPLACE(total_cost,'[^0-9.]','')))/ NULLIF(TRY_TO_NUMBER(REGEXP_REPLACE(total_cost,'[^0-9.]','')),0))) < 0.01
            THEN 'Valid'
        ELSE 'Mismatch'
    END AS roi_validation,
