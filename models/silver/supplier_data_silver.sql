...
cleaned_phone_number AS (
 
SELECT
 
    *,
    
    REGEXP_REPLACE(contact_phone,'[^0-9]','') AS phone_number_clean
...
cleaned_supplier AS (
    ...
    SELECT
        ...
        CASE
            WHEN LOWER(TRIM(contact_email)) RLIKE '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' THEN LOWER(TRIM(contact_email))
            ELSE NULL
        ...
        CASE
            WHEN LENGTH(phone_number_clean) = 11 AND LEFT(phone_number_clean,1) = '1' THEN SUBSTRING(phone_number_clean,2)
            WHEN LENGTH(phone_number_clean) = 10 AND LEFT(phone_number_clean,1) = '1' THEN NULL
            WHEN LENGTH(phone_number_clean) = 10 THEN phone_number_clean
            ELSE NULL
        END AS valid_phone,
    ...
    FROM cleaned_phone_number
 
)
 
SELECT *
FROM cleaned_supplier