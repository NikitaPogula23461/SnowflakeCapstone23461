...
cleaned_store AS (
    ...
    SELECT
        ...
        CASE
            WHEN LOWER(TRIM(email)) RLIKE '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$' THEN LOWER(TRIM(email))
            ELSE NULL
        END AS valid_email,
        ...
        CASE
            WHEN LENGTH(phone_number) = 11 AND LEFT(phone_number,1) = '1' THEN SUBSTRING(phone_number,2)
            WHEN LENGTH(phone_number) = 10 AND LEFT(phone_number,1) = '1' THEN NULL
        ...
        --Store age 
        CASE
            WHEN TRY_TO_DATE(opening_date) IS NOT NULL THEN DATEDIFF(year, TRY_TO_DATE(opening_date), CURRENT_DATE)
            ELSE NULL
        ...
        CASE
            WHEN TRY_TO_NUMBER(sales_target) > 0 THEN (TRY_TO_NUMBER(current_sales) / TRY_TO_NUMBER(sales_target)) * 100
            ELSE NULL
        END AS sales_target_achievement_percentage,
        
        
        CASE
            WHEN TRY_TO_NUMBER(size_sq_ft) > 0 THEN TRY_TO_NUMBER(current_sales) / TRY_TO_NUMBER(size_sq_ft)
            ELSE NULL
        END AS revenue_per_sq_ft,
        
        
        CASE
            WHEN TRY_TO_NUMBER(employee_count) > 0 THEN TRY_TO_NUMBER(current_sales) / TRY_TO_NUMBER(employee_count)
            ELSE NULL
        ...
        CASE
            WHEN TRY_TO_NUMBER(sales_target) > 0 AND (TRY_TO_NUMBER(current_sales) / TRY_TO_NUMBER(sales_target)) * 100 < 90 THEN 'Underperforming'
            ELSE 'Normal'
